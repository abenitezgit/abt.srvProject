...
public class App {
    ...
    public static void main( String[] args )
    {
		...
		calhoy.add(Calendar.DATE, -14);
		Date fecHoy = calhoy.getTime();
		
		calayer.add(Calendar.DATE, -15);
	...
	static void execMov4() {
		try {
			...
			System.out.println("Estado de Conexión DBSource: "+sconn.isConnected());
			
			sqlDB dconn = new sqlDB("172.17.69.8","Ods_Genesys","1433","user_econtact","Vtr.@contact2016",10);
			dconn.open();
			
			System.out.println("Estado de Conexion DBDest (VTR): "+dconn.isConnected());
			...
			if (sconn.isConnected() && dconn.isConnected()) {
				...
				while (rs.next()) {
					...
					psInsertar =  dconn.getConnection().prepareStatement(	"insert into cdr_detail_v2 (" + cols + ") VALUES (" + Variables	+ ")");
	...
	static void execMov3() {
		try {
			...
			System.out.println("Estado de Conexión DBDest (VTR): "+dconn.isConnected());
			...
			if (sconn.isConnected() && dconn.isConnected()) {
				...
				while (rs.next()) {
					...
					psInsertar =  dconn.getConnection().prepareStatement(	"insert into cdr_ext_v2 (" + cols + ") VALUES (" + Variables	+ ")");
	...
	static void execMov2() {
		try {
			...
			System.out.println("Estado de Conexión DBDest: "+dconn.isConnected());
	...
	static void execMov1() {
		try {
			...
			mysqlDB sconn = new mysqlDB("172.17.233.65","navegacion","3306","userNav","u341700",10);
			//mysqlDB sconn = new mysqlDB("172.17.233.65","navegacion","3306","abenitez","Maca-3108",10);
			sconn.open();
			...
			System.out.println("Estado de Conexion DBDest: "+dconn.isConnected());
